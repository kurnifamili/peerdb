...
func (s PeerFlowE2ETestSuitePG) Test_Index_Replication() {
	srcTableName := s.attachSchemaSuffix("test_index_replication")
	dstTableName := s.attachSchemaSuffix("test_index_replication_dst")
	...
	connectionGen := FlowConnectionGenerationConfig{
		FlowJobName:      s.attachSuffix("test_index_replication_flow"),
		TableNameMapping: map[string]string{srcTableName: dstTableName},
		Destination:      s.Peer().Name,
	}
	...
	`, s.suffix, "test_index_replication_dst", "test_index_replication_dst")).Scan(&indexCount)
	require.NoError(s.t, err)
	...
	`, s.suffix, "test_index_replication_dst")).Scan(&exists)
	require.NoError(s.t, err)
